...
struct Longest_Increasing_Subsequence_2D {

	std::vector<int> dp, mx;
	vector<pair<pii, int> > tem;

	Longest_Increasing_Subsequence_2D() {
		dp.resize(N, 1);
	}
	...
	int bs(int x, int y) {
		...
		while (low <= high) {
			int mid = (low + high) >> 1;
			if (tem[mid].ff.ff < x && tem[mid].ff.ss < y) ans = mx[mid], low = mid + 1;
			else high = mid - 1;
		}
		return ans;
	}
...
int solve() {
	...
	Longest_Increasing_Subsequence_2D foo;
	printf("%d\n", foo.get_result(n));
	return 0;
}
